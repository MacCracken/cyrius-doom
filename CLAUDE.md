...
- **`cyrius.lock`**: committed supply-chain anchor — the resolver's own `cyrius deps` output (a clean **6.4.2** resolve is **100 entries**; the 6.1–6.3 stdlib was 37 — the count tracks the pinned toolchain's `lib/` snapshot). `./lib/` is gitignored, so the lock hashes the pinned stdlib + git-override deps. CI checks the lock is present, runs `cyrius deps` (resolves `./lib/` + rewrites the lock from the pinned toolchain), then `cyrius deps --verify` as the unconditional gate — a clean 6.4.2 checkout verifies 100/0. The earlier cycc 6.0.1 lockfile-writer regression (empty lock → `sha256sum` hand-population + guarded verify) was resolved in 0.27.5 by the 6.0.29 pin.
