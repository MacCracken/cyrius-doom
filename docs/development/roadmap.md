...
| **v0.34.6** | **TX-3 — wall texture-V anchored at the view centre** (deferred from v0.34.5) | Anchor wall texture-V at the view centre (vanilla `dc_texturemid`) instead of the floor-truncated wall-top row (`ceil_screen` / `lower_y1` / `mtop`, all `fixed_to_int`-truncated) — a ≤1-texel vertical shimmer as the view rotates. Shape: hoist a per-seg, per-section world constant `v_mid = ((anchor_h + yoff) << 16) - view_z` and use `ystart = v_mid + (y1 - HALF_HEIGHT) * ty_step` per column, keeping `ty_step = fixed_div(FIXED_ONE, col_scale)` (the shipped F06 native V-step) untouched. The four `anchor_h` cases are algebraically the shipped F06/F-R3/F-R4 pegging re-expressed at the view centre, so pegging is preserved by construction — but that must be re-proven, not assumed. **Known intended delta**: the lower section shifts by exactly one `ty_step`, because the shipped `back_floor_screen + 1` anchor is off-by-one-row vs vanilla; that cannot be preserved in a world-space constant and preserving it would defeat the fix. Also subsumes the closed-gap V re-anchor hack and the R-10 clamp-advance. Needs a **VERTICAL-slide metric** — the v0.34.5 harness (`dumpstage.sh`, per-column `tex_u` dump from the shipped engine) is U-only and would extend cleanly by dumping `ty_pos`/`ty_step`. Sequence BEFORE OP-8, which restructures the same per-column V-step. | queued — design in the 2026-07-17 audit §1c |
