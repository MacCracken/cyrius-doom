...
| **v0.33.7** | **Door/lift fidelity follow-ups** (surfaced by the 0.33.5 G-2 review; each pre-existing) | (1) doors open to the *highest* neighbor ceiling; vanilla `P_FindLowestCeilingSurrounding` uses the LOWEST (add `find_lowest_neighbor_ceil`, exclude self) — visible only on multi-neighbor door sectors. (2) **Blazing/turbo speed**: 117/118 blazing doors, 122 blazing lift, 70/71 turbo floor-lower run at base DOOR_SPEED/LIFT_SPEED — add a fast-speed variant. (3) **W1/S1 one-shot latch**: W1/S1/D1 once-only specials re-fire on re-cross/re-use (only an active thinker suppresses them) — a per-linedef used-flag. (4) **G-5** same-side escape-rule zero-band (collision cross-product quantizes to 0 within ~0.01-0.13 map units → a move can skip the ML_BLOCKING/one-sided veto; treat `side==0` as crossing or compute at full precision). | queued |
