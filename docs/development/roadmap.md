...
| **(unslotted)** | **Options → Sound menu, wired live** now that ALSA audio actually plays (0.30.5). The `MENU_OPTIONS` screen (0.30.2) renders `M_SVOL` as a display-only stub; make SFX volume real (a master-gain multiply in the `audio_tick` mix stage — the same hook the deferred per-sound normalization would use) and reflect it in the menu. | new — 2026-06-29 audio revive (0.30.5), user-requested post-release |
| **(unslotted)** | **Audio output hardening, post-0.30.5** (from the 0.30.5 pre-cut review, all confirmed MED/LOW): (1) **HW_PARAMS-fallback SW thresholds** [MED] — on the `audio_set_params` fallback path the kernel picks the buffer but `audio_set_sw_params` still hardcodes `MIX_START=3072`/`stop=8192`; a negotiated buffer <3072 never auto-starts (silence). Read back `BUFFER_SIZE` from the 608-byte `hwp` and derive `start=min(3072, buf*3/4)`, `stop=buf`. Low trigger prob (1024/8192 is standard HDA, accepted on the dev box). (2) **Broaden XRUN recovery** [LOW] — only `-EPIPE` is recovered; `-ESTRPIPE` (suspend/resume) or `-EBADFD` permanently silences the session. On any negative write except `-EAGAIN`, `audio_resume()` (exists unused, `vani-core.cyr:631`) then `audio_prepare`. (3) **Per-sound peak normalization / master-gain** — soft lumps like `DSITEMUP` (±19) play ~6× quieter than gunfire; kept faithful for now (shares the menu-volume hook above). (4) **48000 Hz fallback** — jack also accepts it; needs fractional 11025→48000 resample vs the clean 4× for 44100. (5) **Device-pick virtual-card heuristic** — the capture-sibling test can pick snd-aloop/dummy over the real codec; needs vani CARD_INFO. (6) Distance/positional SFX attenuation; ALSA-vs-PC-speaker double-fire gating (`sound_*` could no-op when `audio_dev!=0`). | new — 2026-06-29 audio revive review |
