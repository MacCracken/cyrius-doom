...
| **v0.34.5** | **Texture world-lock — masked + TX-3** (deferred from 0.34.2; both need extended harness staging) | **Masked-midtexture V4**: grates/two-sided midtextures swim the same way walls did; port the wall ray-cast into `render_masked_one` — needs the `MASKED_ENTRY` to carry the unclipped `rtx1/rty1/rtx2/rty2` + raw `u_v1/u_v2` (layout change, ~120→144-152 B; `render_store_masked` signature + all offsets in lockstep), and a world-lock stage on a two-sided line WITH a real middle texture (linedef 37 has none — pick a grate/bars line or stage a synthetic viewpoint). Retires the now-INFO dead-compute of `uow1/uow2` for one-sided walls. **TX-3**: anchor wall texture-V at the view center (vanilla `dc_texturemid`), not the floor-truncated wall-top row — a ≤1-texel vertical shimmer whose fix is contingent on 0.34.2's V4 (the dominant vertical error was the corrupted `col_scale`→`ty_step`, now fixed); reconcile with the shipped F06 native V-step + pegging (F-R3/F-R4) so fitted walls stay ~byte-identical; needs a VERTICAL-slide metric added to the world-lock harness (`replicate.py` is U-only). Sequence TX-3 BEFORE OP-8 (which restructures the same per-column V-step). | queued — design in the 2026-07-17 audit §1b/§1c |
...
| **v0.34.4** | **Perf batch B — deferred perf-A tail + scaling insurance** | **From perf-A (deferred, higher-risk/complex):** **OP-2** composited-texture cache (**−0.40 ms** + removes the pcache-overflow cliff; lazily composite each texture once, serve later calls as memcpys — `texture_get_column` ~823 ns hot path; `anim_rotate_tex_3` invalidation hook = the SLADRIP-anim fix vehicle), **OP-4** status_render lump/patch caching (~161 µs → est. <30; resolve+slurp all HUD patches at `status_init_font`), **OP-7**/F12 sidedef/flat index caches (~50 µs; SLADRIP prerequisite). **Scaling insurance:** **OP-5** `thing_check_sight` (41 µs × every idle monster × every tick — 8–15 ms/tick extrapolated on E1M7-class maps): REJECT lump as (sector,sector) LOS pre-test + per-linedef bboxes at load + staggered idle wake. **OP-9** clip-band incremental stepping. **OP-10** palette→XRGB LUT in the present blits. **OP-8** per-seg incremental stepping (only AFTER v0.34.5's TX-3 — both restructure the per-column V-step). | queued, bench-gated |
