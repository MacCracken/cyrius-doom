...
| ~~(unslotted)~~ | ~~**Render-consistency audit Bite C — gameplay sweep**~~ **SHIPPED 0.32.0** (2026-07-08: RC-G1 door-entombment reversal, RC-G2 trigger segment-span, RC-G3 real use-ray + blocking veto, RC-G4 closed-portal sight/hitscan, RC-G5 missile spawn check + splash LOS, RC-G7 alloc guards — +8 regression asserts, doors.cyr added to the test harness). **Residuals**: RC-G6 AGNOS menu edge-latch (QEMU-gated); monsters aren't obstruction-checked by closing doors (rides the F15 thing-sector cache); RC-G8 LOW bundle held with 0.28.9–.11 | — |
| **v0.33.0** | **Desktop rendering — native Wayland window backend, using [puka](https://github.com/MacCracken/puka) as the working example.** puka proves the whole path in sovereign Cyrius (zero FFI): a resizable window on Hyprland/wlroots via its `platform/wayland/*` client + the `win_*` seam (`window.cyr`), with keyboard input and damage-aware CPU blits (`fb.cyr`). Doom's version: open a Wayland window, blit the 320×200 palette-expanded frame (integer-scaled, as `framebuf_flip` already does for /dev/fb0), take keys from the Wayland keyboard path — replacing the /dev/fb0-or-GTK-bridge (`scripts/x11view.py`) desktop story. Pulls the v1.0.0 "Wayland display backend" item forward; X11 stays a v1.0.0 option. Mirror puka's seam layout so the fb0/AGNOS/Wayland backends sit behind one interface. | **next** — user-directed 2026-07-08 |
...
| **F-R6** | Palette index 0 treated as transparent everywhere — **psprite + thing-sprite halves FIXED 0.32.0** (the see-through gun: the psprite blitter draws every in-post pixel; the sprite dense buffer gained a fill mask). Remaining: the TEXTURE path (`render_draw_tex_column` / masked columns) still uses index 0 as the unfilled-row key. | Remaining half needs per-column fill tracking through `texture_get_column` (same shape as the sprite fill mask) — ride it with the masked-transparency work; wall-visible impact is minimal (index-0 texels shade to black over black). |
...
| 3 | Wayland display backend | **Pulled forward → v0.33.0** (2026-07-08, user-directed) | puka-pattern sovereign Wayland client; see the v0.33.0 slot |
