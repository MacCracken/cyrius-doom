...
| **(unslotted)** | **Audio output hardening, post-0.30.6** (remaining items; (1) HW_PARAMS-fallback thresholds + (2) ESTRPIPE recovery **shipped in 0.30.6**): (3) **Per-sound peak normalization** or a finer **master-gain curve** — soft lumps like `DSITEMUP` (±19) play ~6× quieter than gunfire; kept faithful for now (the 0.30.6 `sfx_volume` gain is the hook). (4) **48000 Hz fallback** — jack also accepts it; needs fractional 11025→48000 resample vs the clean 4× for 44100. (5) **Device-pick virtual-card heuristic** — the capture-sibling test can pick snd-aloop/dummy over the real codec; needs vani CARD_INFO. (6) **Distance/positional SFX attenuation**; ALSA-vs-PC-speaker double-fire gating (`sound_*` could no-op when `audio_dev!=0`). (7) **Sound-menu polish** (0.30.6 review nice-to-haves): live SFX preview on slider step (device is open before the menu — `audio_play_name`+pump `audio_tick` in `menu_run`); defensive `[0,15]` clamp inside `menu_draw_thermo`; `else`-gate the slider `inc` so simultaneous L+R isn't a net-zero double-step. | updated — 2026-06-29 0.30.6 (2 items shipped, review nice-to-haves folded in) |
