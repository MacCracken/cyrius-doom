...
> **Last refresh**: 2026-06-12 (v0.29.4 — **wall black-holes + walk-through-walls fixed**. Walls: the "void / black holes" family [E1M9 ~70% black, E1M3 center, E1M7 left, E1M1 doorways] was diagnosed by a multi-agent A/B-render workflow as **two texture-resolution bugs, NOT geometry/clip** [overflow, NEAR_CLIP, one-sided clip-drop all empirically refuted]: PNAMES `strlen` over-read of non-terminated 8-byte patch names [161/350 → `patch_lumps=-1` → black] + patch-cache 8192-byte truncation [big door/wall patches lose columns past byte 8192]. All 4 maps now ≤0.1% viewport black. Collision: blockmap cell-index double-shift [`fixed_to_int(asr(.,16))` = asr 32] collapsed every query to cell (0,0) → player walked through every wall; latent since 0.13.0, + a new regression test [full suite 73→75]. Two survivors NOT shipped [closed-sector-clip-inversion HIGH, wall-u-swap-mirror MED] → roadmap. **AGNOS QEMU verified** on the final v0.29.4 binary [doom-smoke + in-game E1M1 sendkey harness: textured walls/floors, viewport-black 0.06% / 104 colors == Linux]. NOTE: built under cycc **6.2.2** — the launcher ignores the 6.1.37 pin, see toolchain row.) | **Refresh cadence**: every release (ideally bumped by the release post-hook).
>
...
**[`VERSION`](../../VERSION)** = `0.29.4` (single source of truth — `cyrius.cyml` reads it via `${file:VERSION}`).
...
| Metric | Value |
|---|---|
| `build/doom` | **602,032 B** (cycc 6.2.2 — see toolchain drift note; +96 B over 0.29.3 for the PNAMES null-terminate loop). `build/doom_agnos` = 581,072 B (+112 B). |
| Unreachable fns (NOP-sled today, real shrink under O3) | 996 / 292,427 B |
| Recovery target under Cyrius O3 real DCE | ~260 KB |
| Frame time | `render_frame` **2.93 ms** / `+sprites` 2.94 ms (E1M1, 0.29.4, cycc 6.2.2). Up from 0.29.3's 2.451 ms — **A/B-isolated to the PNAMES fix**: the renderer now composites and draws the wall textures that were previously unresolved black no-ops, so it does real per-pixel work where it used to skip. NOT the patch-cache bump (render_frame at PCACHE 8192 vs 40960 = 2.926 vs 2.937 ms = variance). ~7.5× headroom on the 22 ms budget. |
...
| `cyrius build src/main.cyr build/doom` | OK, 602,032 B (cycc **6.2.2** — launcher ignores the 6.1.37 pin, see toolchain row) |
| `cyrius build --agnos src/main.cyr build/doom_agnos` | OK, **581,072 B**. **QEMU-verified on the final 0.29.4 binary**: `agnos/scripts/doom-smoke.sh` **PASS** (gnoboot+OVMF+NVMe; serial `cyrius-doom v0.29.4` + `wad loaded`; fb 240 colors), and `agnos/scripts/doom-ingame-smoke.py` **PASS** — menu-driven into E1M1 (`map: V=467…` on serial), screendump shows **textured walls + floors + ceilings, no void**, floor band 11–24 colors/row. Quantitative: in-game viewport-black **0.06%** / **104** distinct colors == the Linux `--ppm` render (0.06% / 104) — rendering-equivalent (the sendkey viewpoint differs only slightly from spawn). No `--agnos` codegen hazard (constant/alloc/cold-path changes; 6.1.37 3-op fold holds). |
| `cyrius test tests/doom.tcyr` (WAD-free, CI subset) | 37/37 |
| `./build/test_doom wad/DOOM1.WAD` (full) | **75/75** (+2: a collision-blockmap regression group — spawn is valid + the spawn sweep blocks somewhere, i.e. not walk-through-walls) |
| `fuzz_wad` / `fuzz_fixed` | **1000 / 50000 clean** (re-run this cycle — 0.29.4 touched `texture_init` / patch cache, downstream of the WAD parser; re-ran as the work-loop fuzz gate). |
| `./build/doom wad/DOOM1.WAD --ppm` | E1M1 + automap + intermission PPMs at 192,015 B each; map summary `V=467 L=475 SD=648 S=85 SG=732 SS=237 N=236 T=138`. **Walls now resolve + composite their textures** — viewport-black (rows 0–167) E1M1 7.6%→**0.1%**, E1M3 10.5%→**0.0%** (111 colors), E1M7 17.8%→**0.1%**, E1M9 **70.7%→0.1%**. |
