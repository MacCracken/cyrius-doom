...
| `cyrius build --agnos src/main.cyr build/doom_agnos` | OK, 580,960 B. `agnos/scripts/doom-smoke.sh` **PASS on the final 0.29.3 binary** (QEMU gnoboot+OVMF+NVMe; serial `cyrius-doom v0.29.3` + `wad loaded`; fb 240 colors). In-game E1M1 via `agnos/scripts/doom-ingame-smoke.py` (promoted this cut; built-in textured-flats gate): **PASS** — map stats on serial, 11–24 distinct colors per floor-band row (smear = 1–4), screendump shows **textured floors with depth lighting**, and the framebuffer is **99.99% pixel-identical to the Linux render at 4× block scale** (7/64,000 px differ — idle-monster anim tick skew). Earlier in the cut, a serial probe on the distance-fix build confirmed `fixed_mul(VIEW_HEIGHT, PROJ_DIST)=429916160` = Linux (no 6.2.2 `--agnos` fold regression). The stock smoke only reaches TITLEPIC — the in-game harness is what exercises the flat renderer. |
