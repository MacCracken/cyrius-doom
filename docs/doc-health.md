...
> **Last refresh**: 2026-07-08 v0.31.5 prep (**Bite A shipped**: CHANGELOG `[0.31.5]` gained the six audit fixes + 21 asserts + A/B perf note; state.md → 0.31.5 [vani vendored → `vendor/vani-core.cyr` 0.9.9, lock 34/0, cycc 6.4.29 drift metrics, gates re-run, slot row]; roadmap Bite A row + wall-path #6 + 0.28.7 item 0 marked SHIPPED, 0.28.6 promoted to next; CLAUDE.md stale lock-count inlines replaced with state.md pointers. Earlier same day: **render-consistency audit** — new dated artifact `docs/audit/2026-07-08-render-consistency.md` [21 findings], roadmap audit section + Bite rows. Prior refresh: 2026-06-29 v0.30.7). | **Refresh cadence**: opportunistic — update the affected row when the underlying doc is touched. No periodic sweep cron.
>
...
| `CHANGELOG.md` | 2026-07-08 | ✅ Fresh | **Source of truth per CLAUDE.md.** Through **v0.31.5** (vani-core vendoring + render-audit Bite A: 6 fixes, +21 asserts, A/B perf note under drifted cycc 6.4.29). Refreshed every release. |
| `CLAUDE.md` | 2026-07-08 | ✅ Fresh | Durable content only (state delegated to `state.md`). 2026-07-08: the two inlined lock-entry counts (Work-Loop §7 + CI/Release) replaced with `state.md` pointers — they'd gone stale twice (0.31.0 stdlib growth, 0.31.5 vani vendoring); per the file's own no-volatile-state rule. Project-identity / Goal / Process / Rules / Cyrius Conventions all durable. |
...
| `2026-07-08-render-consistency.md` | 2026-07-08 | 🔵 Dated artifact | **Render-consistency audit** (walls/flats/sprites + 14-module sweep). 21 findings with staged-viewpoint PPM evidence + repro coordinates: RC-S1/S2 sprite deletion/x-ray (elevates 0.28.6), RC-S3 12 invisible thing types, RC-S4 atan2, RC-S5 sprite shredding, RC-W1 sky-vs-sky walls, RC-W2 closed-door seam confirmed, RC-G1–G8 gameplay sweep. Drives the roadmap Bite A/C rows + 0.28.6-before-0.28.5 recommendation. |
