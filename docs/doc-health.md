...
> **Last refresh**: 2026-06-29 (v0.30.6 — **SFX volume + Sound menu + bsp 1.2.0**: CHANGELOG / state.md / roadmap.md rows touched; CHANGELOG gained the `[0.30.6]` entry; state.md → 0.30.6 [binary 621,080 B, render 2.950 ms, bsp 1.2.0, Sound menu]; roadmap marked 2 audio-hardening items shipped + folded in the 0.30.6 review nice-to-haves. Prior refresh: 2026-06-29 v0.30.5 [audio revive]). | **Refresh cadence**: opportunistic — update the affected row when the underlying doc is touched. No periodic sweep cron.
>
...
| `CHANGELOG.md` | 2026-06-29 | ✅ Fresh | **Source of truth per CLAUDE.md.** Through **v0.30.6** (SFX volume + Options→Sound menu, ESTRPIPE/start-threshold ALSA hardening, bsp 1.1.5→1.2.0). Refreshed every release. |
...
| File | Last touched | Status | Action |
|---|---|---|---|
| `state.md` | 2026-06-29 | ✅ Fresh | **Rotates every release.** Current to v0.30.6 — **SFX volume + Sound menu + bsp 1.2.0**: binary 621,080 B (agnos 607,680 B), `render_frame` 2.950 ms, lock 37/0 (bsp row moved). Live Options→Sound slider (master `sfx_volume` gain); ESTRPIPE recovery + fallback-threshold gating. Gates re-run on Linux (63/63 + 101/101, fuzz 1000/50000, Sound-menu PPM); AGNOS QEMU not gated. v0.30.6 slot row added; 0.30.5 → shipped. |
| `roadmap.md` | 2026-06-29 | ✅ Fresh | **Re-slotted at v0.28.4 cut** (0.28.5–.11 carry the Black Book parity/perf themes). Floor-render + shooting-overhaul deferrals folded in as unslotted rows. v0.30.1 wall-path/psprite/flash items RESOLVED. **v0.30.6**: the Options→Sound-menu row + 2 audio-hardening items (HW_PARAMS-fallback thresholds, ESTRPIPE recovery) **shipped**; the remaining audio-hardening row now carries normalization/48 kHz/virtual-card/distance + the 0.30.6 review nice-to-haves. Slot numbers vs shipped 0.29.x/0.30.x acknowledged stale — renumbering deferred. |
