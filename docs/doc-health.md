...
> **Last refresh**: 2026-06-29 (v0.30.7 — **positional/stereo SFX + Sound-menu live preview**: CHANGELOG / state.md / roadmap.md rows touched; CHANGELOG gained the `[0.30.7]` entry; state.md → 0.30.7 [binary 623,520 B, render 2.956 ms, positional audio]; roadmap marked distance-attenuation + menu polish shipped, flagged the remaining items (48 kHz / virtual-card / normalization / double-fire) with deferral reasons. Prior refresh: 2026-06-29 v0.30.6 [SFX volume + Sound menu + bsp 1.2.0]). | **Refresh cadence**: opportunistic — update the affected row when the underlying doc is touched. No periodic sweep cron.
>
...
| `CHANGELOG.md` | 2026-06-29 | ✅ Fresh | **Source of truth per CLAUDE.md.** Through **v0.30.7** (positional/stereo SFX — distance attenuation + pan, `audio_play_at`; Sound-menu live preview + polish). Refreshed every release. |
